...
def create_abundance_plots(normalized_data_sum, output_dir, panel_b_sources, db_colors, suffix=""):
    ...
    # Sort by non-zscore abundance space and assign order
    peptideatlas_data_sum = peptideatlas_data_sum.sort_values('log_abundance')
    ...
    # Build non-zscore comparison value per source:
    # - PeptideAtlas and external DBs: log_abundance
    # - quantm: PA-like log abundance when available (same space as density no-zscore plot)
    dot_plot_data_sum['display_value'] = dot_plot_data_sum['log_abundance']
    if 'abundance_norm_pa_like' in dot_plot_data_sum.columns:
        qmask = dot_plot_data_sum['source'].eq('quantm (Plasma/Serum)')
        qvals = pd.to_numeric(dot_plot_data_sum.loc[qmask, 'abundance_norm_pa_like'], errors='coerce').fillna(0.0)
        dot_plot_data_sum.loc[qmask, 'display_value'] = np.log10(qvals + 1.0)

    # Group by gene and source, take median value in this non-zscore space
    dot_plot_data_sum = dot_plot_data_sum.groupby(['gene', 'source'], as_index=False).agg({
        'display_value': 'median',
        'order': 'first'
    })
    ...
    for source in plot_order:
        ...
        if len(source_data) > 0:
            ax.scatter(source_data['order'], source_data['display_value'],
                     label=source, alpha=0.7, s=8, color=db_colors.get(source, 'gray'))
    
    ax.set_xlabel('Protein Rank (by PeptideAtlas log abundance)', fontsize=12)
    ax.set_ylabel('Non-zscore abundance space (log10 abundance)', fontsize=12)
    ax.set_title('(B) Cross-database abundance vs PeptideAtlas baseline\n(non-zscore; quantm uses PA-like abundance)',
    ...
    # PLOT 2/3/4 legacy z-score figures removed by request:
    # - peptideatlas_msstats_summed.*
    # - msstats_abundance_distribution.*
    # - msstats_reference_peptideatlas_cloud.png / msstats_reference_data.csv
    ...
    if len(quantm_data) > 0 and len(pa_data) > 0:
        ...
        if len(merged_data) > 0:
            # Legacy z-score density outputs are intentionally disabled.
            # Keep only PA-like no-zscore density variants below.
            pass
            ...
            if 'abundance_norm_pa_like' in quantm_data.columns:
                ...
                if len(quantm_norm) > 0:
                    ...
                    merged_data_pa_like = quantm_norm[['gene', 'z_score_quantm_pa_like', 'abundance_norm_pa_like', 'log_abundance']].merge(
                        pa_data[['gene', 'z_score', 'abundance', 'log_abundance']],
                        on='gene',
                        how='inner'
                    ).rename(columns={
                        ...
                        'log_abundance_x': 'log_abundance_quantm_pa_like',
                        'log_abundance_y': 'log_abundance_peptideatlas',
                    })
                    # Defensive fallback in case pandas merge column naming differs.
                    if 'log_abundance_peptideatlas' not in merged_data_pa_like.columns and 'log_abundance' in merged_data_pa_like.columns:
                        merged_data_pa_like = merged_data_pa_like.rename(columns={'log_abundance': 'log_abundance_peptideatlas'})
                    if len(merged_data_pa_like) > 0:
                        ...
                        support_variants = [
                            ("all_proteins", merged_with_support['n_datasets_plasma'] >= 0),
                        ...
                        for vname, vm in support_variants:
                            ...
                            if len(vdf) == 0:
                                continue
                            xnz = vdf['log_abundance_peptideatlas'].to_numpy(float)
                            ynz = vdf['log_abundance_quantm_pa_like'].to_numpy(float)
                            valid_nz = np.isfinite(xnz) & np.isfinite(ynz)
                            xnz = xnz[valid_nz]
                            ynz = ynz[valid_nz]
                            if len(xnz) == 0:
                                continue

                            # Improve spatial spread for visualization only:
                            # robust min-max normalize each axis to [0, 1].
                            def _robust_norm01(arr):
                            ...
                            x_plot = _robust_norm01(xnz)
                            y_plot = _robust_norm01(ynz)

                            # Non-linear display transform (monotonic) to spread low-end crowding
                            # without changing underlying values or correlation calculations.
                            # asinh-stretch keeps axis in [0,1] and expands near-zero region.
                            alpha = 25.0
                            denom = np.arcsinh(alpha)
                            x_plot = np.arcsinh(alpha * x_plot) / denom
                            y_plot = np.arcsinh(alpha * y_plot) / denom

                            fig_nz, ax_nz = plt.subplots(figsize=(10, 8))
                            hb_nz = ax_nz.hexbin(
                                x_plot,
                                y_plot,
                                gridsize=70,
                            ...
                            cb_nz = plt.colorbar(hb_nz, ax=ax_nz)
                            cb_nz.set_label('Protein density (log-scaled)', fontsize=12)
                            r_nz = np.corrcoef(xnz, ynz)[0, 1]
                            ax_nz.text(
                                0.05, 0.95,
                                f'Pearson r = {r_nz:.3f}\\nn = {len(xnz)} proteins',
                                transform=ax_nz.transAxes, fontsize=12, verticalalignment='top',
                                bbox=dict(boxstyle='round', facecolor='white', alpha=0.85)
                            )
                            ax_nz.set_xlabel('PeptideAtlas log10(abundance + 1), display-normalized [0,1]', fontsize=11)
                            ax_nz.set_ylabel('quantm PA-like log10(abundance + 1), display-normalized [0,1]', fontsize=11)
                            ax_nz.set_title(
                                f"Density Plot (no z-score): quantm (PA-like) vs PeptideAtlas\n{vname.replace('_', ' ')}",
                                fontsize=14, fontweight='bold'
                            )
                            ax_nz.grid(True, alpha=0.3)
                            plt.tight_layout()
                            fpng_nz = f"density_plot_quantm_vs_peptideatlas_pa_like_{vname}_no_zscore{suffix}.png"
                            plt.savefig(output_dir / fpng_nz, bbox_inches='tight', dpi=300)
                            plt.close()
                            print(f"  Saved: {fpng_nz}")
                            # Alternate version requested: explicit heatmap-like 2D histogram with red palette.
                            fig_alt, ax_alt = plt.subplots(figsize=(10, 8))
                            h = ax_alt.hist2d(
                                x_plot,
                                y_plot,
                                bins=80,
                            ...
                            cb_alt = plt.colorbar(h[3], ax=ax_alt)
                            cb_alt.set_label('Protein density (log-scaled)', fontsize=12)
                            ax_alt.text(
                                0.05, 0.95,
                                f'Pearson r = {r_nz:.3f}\nn = {len(xnz)} proteins',
                                transform=ax_alt.transAxes, fontsize=12, verticalalignment='top',
                                bbox=dict(boxstyle='round', facecolor='white', alpha=0.85)
                            )
                            ax_alt.set_xlabel('PeptideAtlas log10(abundance + 1), display-normalized [0,1]', fontsize=11)
                            ax_alt.set_ylabel('quantm PA-like log10(abundance + 1), display-normalized [0,1]', fontsize=11)
                            ax_alt.set_title(
                                f"2D Heatmap (no z-score): quantm (PA-like) vs PeptideAtlas\n{vname.replace('_', ' ')}",
                                fontsize=14, fontweight='bold'
                            )
                            ax_alt.grid(True, alpha=0.2)
                            plt.tight_layout()
                            falt = f"alt_2d_histogram_pa_like_{vname}_no_zscore{suffix}.png"
                            plt.savefig(output_dir / falt, bbox_inches='tight', dpi=300)
                            plt.close()
                            print(f"  Saved: {falt}")
                            fcsv_nz = f"density_plot_quantm_vs_peptideatlas_pa_like_{vname}_no_zscore_data{suffix}.csv"
                            vdf.to_csv(output_dir / fcsv_nz, index=False)
                            print(f"  Saved: {fcsv_nz}")
    
    # PLOT 6: Scatter plot with PeptideAtlas as reference line and other databases as points
    # Y values are relativized to [0,1] per source for direct shape comparison.
    print("\nCreating scatter plot with PeptideAtlas as reference and other databases on top (Y normalized 0-1)...")
    pa_ref = normalized_data_sum[normalized_data_sum['source'] == 'PeptideAtlas'].copy()
    
    if len(pa_ref) > 0:
        # Order by PeptideAtlas abundance trend
        pa_ref = pa_ref.sort_values('log_abundance')
        ...
        if len(other_sources) > 0:
            ...
            pa_ref_grouped = pa_ref.groupby('gene', as_index=False).agg({
                'log_abundance': 'median',
                'order': 'first'
            })
            # Relativize to [0,1] for visual comparability
            y_pa = pa_ref_grouped['log_abundance'].to_numpy(float)
            if np.nanmax(y_pa) > np.nanmin(y_pa):
                pa_ref_grouped['y01'] = (y_pa - np.nanmin(y_pa)) / (np.nanmax(y_pa) - np.nanmin(y_pa))
            else:
                pa_ref_grouped['y01'] = 0.0
            pa_ref_grouped = pa_ref_grouped.sort_values('order')
            ax.plot(pa_ref_grouped['order'], pa_ref_grouped['y01'],
            ...
            for source in other_sources:
                ...
                if len(source_data) > 0:
                    ...
                    if len(source_grouped) > 0:
                        y = source_grouped['log_abundance'].to_numpy(float)
                        if np.nanmax(y) > np.nanmin(y):
                            y01 = (y - np.nanmin(y)) / (np.nanmax(y) - np.nanmin(y))
                        else:
                            y01 = np.zeros_like(y, dtype=float)
                        ax.scatter(source_grouped['order'], y01,
            ...
            if len(quantm_data) > 0:
                ...
                if len(quantm_grouped) > 0:
                    yq = quantm_grouped['log_abundance'].to_numpy(float)
                    if np.nanmax(yq) > np.nanmin(yq):
                        yq01 = (yq - np.nanmin(yq)) / (np.nanmax(yq) - np.nanmin(yq))
                    else:
                        yq01 = np.zeros_like(yq, dtype=float)
                    ax.scatter(quantm_grouped['order'], yq01,
                             label='quantm (Plasma/Serum)', alpha=0.7, s=8, 
                             color=db_colors.get('quantm (Plasma/Serum)', '#9b59b6'), zorder=5)
            
            ax.set_xlabel('Protein Rank (by PeptideAtlas log abundance)', fontsize=12)
            ax.set_ylabel('Relative abundance (0-1 per source)', fontsize=12)
            ax.set_title('Protein Abundance: PeptideAtlas as Reference\n(PeptideAtlas line, other databases points; Y normalized 0-1 per source)',
                        fontsize=14, fontweight='bold')
            ax.legend(title='Data Source', loc='best', fontsize=10)
            ax.grid(True, alpha=0.3, axis='y')
            ax.set_xticks([])
